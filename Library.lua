...
local Templates = {
	...
	Window = {
		...
		MinSize = UDim2.fromOffset(720, 600),
		MaxSize = UDim2.fromScale(1440, 1200),
...
if identifyexecutor then
	...
	FetchIcons, Icons = pcall(function()
		if not script:WaitForChild("Lucide") then
			warn("! You are running inside Studio; Please place a module parented to the Obsidian module named 'Lucide' with the code at https://create.roblox.com/store/asset/15279939717/lucideroblox !")
			
			script:Destroy()
		end
		
		return require("@self/Lucide") :: IconModule
	end)
end
...
function Library:MakeResizable(UI: GuiObject, DragFrame: GuiObject, Callback: () -> ()?, CustomMinSize:Vector2, CustomMaxSize:Vector2)
	local StartPos
	local FrameSize
	local Dragging = false
	local Changed
...
function Library:CreateWindow(WindowInfo)
	...
	WindowInfo.Size = UDim2.fromOffset(
		math.clamp(WindowInfo.Size.X.Offset, (WindowInfo.MinSize and typeof(WindowInfo.MinSize) == "Vector2") and WindowInfo.MinSize.X or Library.MinSize.X, MaxX),
		math.clamp(WindowInfo.Size.Y.Offset, (WindowInfo.MinSize and typeof(WindowInfo.MinSize) == "Vector2") and WindowInfo.MinSize.Y or Library.MinSize.Y, MaxY)
	)
	...
	do
		...
		if WindowInfo.Resizable then
			...
			end, (WindowInfo.MinSize and typeof(WindowInfo.MinSize) == "Vector2") and WindowInfo.MinSize or nil, (WindowInfo.MinSize and typeof(WindowInfo.MinSize) == "Vector2") and WindowInfo.MaxSize or nil)
		end
