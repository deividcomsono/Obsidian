...
function Library:MakeResizable(UI: GuiObject, DragFrame: GuiObject, Callback: () -> ()?, MinSize:Vector2, MaxSize:Vector2)
	local StartPos
	local FrameSize
	local Dragging = false
	local Changed
	...
	Library:GiveSignal(UserInputService.InputChanged:Connect(function(Input: InputObject)
		...
		if Dragging and IsHoverInput(Input) then
			...
			UI.Size = UDim2.new(
				FrameSize.X.Scale,
				math.clamp(FrameSize.X.Offset + Delta.X, typeof(MinSize) == "Vector2" and MinSize.X or Library.MinSize.X, typeof(MaxSize) == "Vector2" and MaxSize.X or math.huge),
				FrameSize.Y.Scale,
				math.clamp(FrameSize.Y.Offset + Delta.Y,  typeof(MinSize) == "Vector2" and MinSize.Y or Library.MinSize.Y, typeof(MaxSize) == "Vector2" and MaxSize.Y or math.huge)
			)
			if Callback then
...
function Library:CreateWindow(WindowInfo)
	...
	do
		...
		if WindowInfo.Resizable then
			...
			end, WindowInfo.MinSize or nil, WindowInfo.MaxSize or nil)
		end
